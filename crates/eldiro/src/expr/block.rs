...
impl Block {
	...
	pub(super) fn eval(&self, env: &Env) -> Result<Val, String> {
		...
		// We can safely unwrap here because we have already checked whether self.stmts is empty.
...
mod tests {
	...
	#[test]
	fn parse_block_with_empty_whitespace() {
		assert_eq!(Block::new("{   }"), Ok(("", Block { stmts: Vec::new() })))
	}
	...
	#[test]
	fn block_with_multiple_binding_defs() {
		assert_eq!(
			Block {
				stmts: vec![
					Stmt::BindingDef(BindingDef {
		...
		assert_eq!(
			Block {
				stmts: vec![
					...
					Stmt::Expr(Expr::Number(Number(3))),
	...
	#[test]
	fn eval_block_using_bindings_from_parent() {
